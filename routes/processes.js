...
router.post("/clients", isLoggedIn, function(req, res) {
  //respond as POST eventhough same route. USING REST API
  ...
  totalSubQuantity =
    subQuantityButton + subQuantityCollar + subQuantityBody + subQuantitySleeve;
  estimatedCost =
    buttonCost * subQuantityButton +
    collarCost * subQuantityCollar +
    bodyCost * subQuantityBody +
    sleeveCost * subQuantitySleeve;
  estimatedSell =
    buttonSell * subQuantityButton +
    collarSell * subQuantityCollar +
    bodySell * subQuantityBody +
    sleeveSell * subQuantitySleeve;
...
router.get("/clients", isLoggedIn, function(req, res) {
  ...
  Process.find({}, function(err, processesData) {
...
router.put("/clients/:lotDataId", function(req, res) {
  Process.findByIdAndUpdate(req.params.lotDataId, req.body.editLot, function(
    err,
    updatedLot
  ) {
    if (err) {
...
router.get("/clients/:lotDataId", isLoggedIn, function(req, res) {
  //SHOW RESTFUL API - WHICH SHOWS MORE INFO ABOUT SOMETHING
